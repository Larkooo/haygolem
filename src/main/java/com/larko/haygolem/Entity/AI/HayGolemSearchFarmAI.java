...
public class HayGolemSearchFarmAI extends Goal
{
    ...
    public boolean canUse()
    {
        ...
        else
        {
            ...
            if (FarmManager.findByUuid(this.hayGolem.farm) != null && !FarmManager.findByUuid(this.hayGolem.farm).isWithinBounds(this.hayGolem.blockPosition()))
                return true;
            else if (FarmManager.findByUuid(this.hayGolem.farm) == null && this.searchForDestination())
                return true;
            else
                return false;
        }
    ...
    {
        return this.timeoutCounter >= -this.maxStayTicks && this.timeoutCounter <= 1200 && this.hayGolem.farm != null && !FarmManager.findByUuid(this.hayGolem.farm).isWithinBounds(this.hayGolem.blockPosition());
    }

    @Override
    public void start()
    {
        BlockPos dest = FarmManager.findByUuid(this.hayGolem.farm).getCenter();
    ...
    public void tick()
    {
        if (!FarmManager.findByUuid(this.hayGolem.farm).isWithinBounds(this.hayGolem.blockPosition()))
        {
            ++this.timeoutCounter;
    ...
    private boolean searchForDestination()
    {
        ...
        {
            this.hayGolem.farm = closest.getUuid();
            FarmManager.findByUuid(this.hayGolem.farm).golemsCount++;
            return true;
        }
